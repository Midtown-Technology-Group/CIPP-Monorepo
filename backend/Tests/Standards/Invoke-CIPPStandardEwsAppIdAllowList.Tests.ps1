...
Describe 'Invoke-CIPPStandardEwsAppIdAllowList' {
    ...
    It 'treats ordering, casing, duplicates, and UI value wrappers as equivalent' {
        Mock New-ExoRequest { [PSCustomObject]@{ EwsEnabled = $true; EwsAllowedAppIDs = "$AppId1,$($AppId2.ToUpperInvariant())" } }
    ...
    It 'writes the complete normalized list and verifies readback' {
        Mock New-ExoRequest {
            ...
            if ($script:ReadCount++ -eq 0) { return [PSCustomObject]@{ EwsEnabled = $null; EwsAllowedAppIDs = $null } }
            return [PSCustomObject]@{ EwsEnabled = $true; EwsAllowedAppIDs = "$AppId1,$AppId2" }
        }
        $script:ReadCount = 0
        Invoke-CIPPStandardEwsAppIdAllowList -Tenant $Tenant -Settings @{ remediate = $true; AllowedAppIds = @($AppId2, $AppId1) }
        Should -Invoke New-ExoRequest -Times 1 -Exactly -ParameterFilter {
            $cmdlet -eq 'Set-OrganizationConfig' -and $cmdParams.EwsEnabled -eq $true -and
            $cmdParams.EwsAllowedAppIDs -eq "$AppId1,$AppId2" -and $UseSystemMailbox -eq $true
        }
    ...
    It 'alerts when the current policy differs' {
        ...
        Invoke-CIPPStandardEwsAppIdAllowList -Tenant $Tenant -Settings @{ alert = $true; standardId = 'standard-1'; AllowedAppIds = @($AppId1) }
        Should -Invoke Write-StandardsAlert -Times 1 -Exactly -ParameterFilter { $standardName -eq 'EwsAppIdAllowList' -and $standardId -eq 'standard-1' }
    }
    ...
    It 'reports normalized current and expected values' {
        ...
        Should -Invoke Set-CIPPStandardsCompareField -Times 1 -Exactly -ParameterFilter {
            $FieldName -eq 'standards.EwsAppIdAllowList' -and $CurrentValue.EwsEnabled -eq $true -and $ExpectedValue.EwsAllowedAppIDs[0] -eq $AppId1
        }
